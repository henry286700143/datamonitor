...
function drawDimensionChart(chartSelectField){
	...
	if(chartSelectField['merge']=="1"){//维度合并是
		initChart('chartContainer',data, field, chartSelectField);
	}
	if(chartSelectField['merge']=="2"){//纬度合并否
...
/**
 * 生成历史图表
 * @param historyChartField（数组）
 */
function getHistoryChart(historyChartField){
	$('#historyChartArea').html('');
	for (var z = 0; z < historyChartField.length; z++) {
		$('#historyChartArea').append('<div id="historyChartContainer'+z+'"></div>');
		var url = historyChartField[z]["url"];
		var dimField = historyChartField[z]["dimField"];
		var dataField = historyChartField[z]["dataField"];
		var charData = new Array();
		//console.log(data['data']);
		if(url == ""){//如果url为空获取当前数据
			charData = data['data'];
		} else {
			$.ajax({
				type : "post",
		...
		if(charData != null){
			...
			var chartLegend = new Array();
			var chartXAxis = new Array();
			var chartSeries = new Array();
			
			for(var key in dimField) {
				var xAxis = new Object();
				xAxis["boundaryGap"] = false;
				xAxis["type"] = "category";
				var xData = new Array();
				for (var i = 0; i < charData.length; i++) {
					xData.push(charData[i][key]);
				}
				xAxis["data"] = xData;
				chartXAxis.push(xAxis);
			}
			
			for(var key in dataField) {
				chartLegend.push(dataField[key]);
				var series = new Object();
				var yData = new Array();
				for (var i = 0; i < charData.length; i++) {
					yData.push(charData[i][key]);
				}
				series["data"] = yData;
				series["name"] = dataField[key];
				series["type"] = "line";
				chartSeries.push(series);
			}
			createChart('historyChartContainer'+z, chartLegend, chartXAxis, chartSeries);
		}
	}
